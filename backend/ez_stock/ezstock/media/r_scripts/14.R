...
for (i in 1:nrow(boors_data)+1){
  ...
  if ((is.na(P_per_E) == FALSE) & (is.na(EPS) == FALSE) & (is.na(current_price) == FALSE)) {
    ...
    if ((real_price < current_price)) {
      ...
      darsad_sood_dehi <- (((current_price - real_price) / current_price) *100)
      
      if ((darsad_sood_dehi > 1) &(darsad_sood_dehi < 50) ) {
...
fig <- plot_ly(data, x = ~y1, y = ~x1, type = 'bar',
               ...
               marker = list(color = 'rgb(217, 127, 127)',
                             line = list(color = 'rgb(0,0,0)', width = 0.5)))
fig <- fig %>% layout(font = t,title = "سهم هایی که قیمت کنونی آنها بیشتر از قیمت واقعی آنهاست",
                      xaxis = list(title = "نام نماد"),
                      yaxis = list(title = "درصد ضرر دهی"))
